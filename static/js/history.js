...
class HistoryManager {
    ...
    setupHoverEffects() {
        // Standard CSS hover effects are sufficient, no JS tilt needed
        // This prevents the layout issues we saw earlier
    }
    ...
    deleteFile(sessionId, filename) {
        try {
            // Update local state
            const sessionIndex = this.history.findIndex(s => s.session_id === sessionId);
            if (sessionIndex !== -1) {
                const session = this.history[sessionIndex];
                session.files = session.files.filter(f => f.filename !== filename);
                session.file_count = session.files.length;
                    ...
                    gsap.to(fileElement, {
                        ...
                        onComplete: () => {
                            ...
                            if (session.file_count > 0) {
                                ...
                                this.loadHistory(); // Simplest way to clean up headers
                            }
                        }
                    });
                }

                this.showToast('File deleted from history', 'success');

                // Try to delete from server too (optional, don't block on error)
                fetch(`/api/history/delete/file/${sessionId}/${filename}`, { method: 'DELETE' }).catch(() => { });
            }
        } catch (error) {
            ...
            if (sessionElement) {
                gsap.to(sessionElement, {
                    ...
                    onComplete: () => {
                        ...
                        this.loadHistory(); // Clean up headers
                    }
                });
            }

            this.showToast('Session deleted from history', 'success');

            // Try to delete from server too
            fetch(`/api/history/delete/${sessionId}`, { method: 'DELETE' }).catch(() => { });
    ...
    deleteBatch() {
        try {
            ...
            this.selectedFiles.forEach(id => {
                ...
                const session = updatedHistory.find(s => s.session_id === sessionId);
                if (session) {
                    session.files = session.files.filter(f => f.filename !== filename);
                    session.file_count = session.files.length;
                }
            ...
            this.selectedSessions.forEach(sessionId => {
                updatedHistory = updatedHistory.filter(s => s.session_id !== sessionId);
            });

            // Clean up empty sessions
            updatedHistory = updatedHistory.filter(s => s.file_count > 0);
    ...
    downloadSelected() {
        ...
        if (confirm(`Download ${totalItems} selected items? (Note: Files must still exist on server)`)) {
    ...
    confirmDeleteSession(sessionId) {
        if (confirm('Are you sure you want to delete this entire session from history?')) {
    ...
    confirmDeleteSelected() {
        ...
        if (confirm('Are you sure you want to delete selected items from history?')) {
            this.deleteBatch();
        }
    }
